    ...
    SettingsScaffold(stringResource(R.string.settings_section_data), onBack) {
        ...
        SettingsGroup("Аудиозаписи") {
            ...
            if (settings.audioRecording != AudioRecording.NONE) {
                ...
                SettingsNote(
                    "Записи лежат во внутренней папке приложения и слушаются на экране " +
                        "разбора урока. Минута занимает около 2 МБ; в режиме «только свою речь» " +
                        "меньше, потому что тишина не пишется."
                )
            }
        }
