    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                Text(
                    text = if (withResults) {
                        "Ошибки и записи о расходе исчезнут — просядут рекомендации тем и " +
                            "цифры расходов за месяц. Пропадут и слова, впервые встреченные " +
                            "в этом уроке, вместе с историей повторений — даже если вы их уже выучили."
                    } else {
...
fun ClearRecordingsDialog(
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Очистить записи за месяц?") },
        text = {
            Text(
